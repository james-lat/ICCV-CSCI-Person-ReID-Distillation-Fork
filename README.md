...
#### Train 
Training code is available in `Script/image_batch.sh` (image) and `Script/image_batch_newton.sh` (videos) and other ablations are available in `Script/run.sh`. Each dataset has some really good performing color profiles Highligted in Logs `outputs/Color Results`.   
