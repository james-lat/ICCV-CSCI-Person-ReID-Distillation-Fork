...
from model.utils import inflate, pooling, c3d_blocks, nonlocal_blocks

__all__ = ['C2DResNet50' ,'AP3DNLResNet50', 'NLResNet50'] 
...
class ResNet503D(nn.Module):
    def __init__(self, block, c3d_idx, nl_idx, temperature=4, contrastive_att=True, global_max_pooling=True, **kwargs):
        super().__init__()
        self.block = block
        ...
        self.global_max_pooling = global_max_pooling
        if self.global_max_pooling:
            self.pooling = pooling.MaxAvgPooling()
            self.bn = nn.BatchNorm1d(2048  * 2)
        else:
    ...
    def forward(self, x, cloth_id = None, **kwargs):
        # print(x.shape)
        # x = x.squeeze(0)
...
def C2DResNet50(**kwargs):
    c3d_idx = [[],[],[],[]]
    nl_idx = [[],[],[],[]]
    return ResNet503D(c3d_blocks.APP3DC, c3d_idx, nl_idx, **kwargs)

def AP3DNLResNet50(**kwargs):
    c3d_idx = [[],[0, 2],[0, 2, 4],[]]
    nl_idx = [[],[1, 3],[1, 3, 5],[]]
    return ResNet503D(c3d_blocks.APP3DC, c3d_idx, nl_idx, **kwargs)

def NLResNet50(**kwargs):
    c3d_idx = [[],[],[],[]]
    nl_idx = [[],[1, 3],[1, 3, 5],[]]
    return ResNet503D(c3d_blocks.APP3DC, c3d_idx, nl_idx, **kwargs)
...
if __name__ == "__main__":
    ...
    wt = "logs/CAL_CASIA_v5/c2dres50/best_model.pth.tar"
    checkpoint = torch.load(wt)
    model = C2DResNet50()
    ...
    wt = "logs/CAL_CASIA_v5/ap3dnlres50/best_model.pth.tar"
    checkpoint = torch.load(wt)
    model = AP3DNLResNet50()
    model.load_state_dict(checkpoint['model_state_dict'], strict=True)
