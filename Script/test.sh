...
COLOR=44
WT=logs/LTCC/ltcc+_Co-44-NW16-1245/eva02_img_extra_token_best.pth

# EVA-attribure.train:  CC:  CMC curve, Rank-1  :48.2%  Rank-5  :60.2%  Rank-10 :66.6%  
# EVA-attribure.train:  CC:  mAP Acc. :24.4%
# EVA-attribure.train:  General:  CMC curve, Rank-1  :83.4%  Rank-5  :89.0%  Rank-10 :91.7%  
# EVA-attribure.train:  General:  mAP Acc. :47.8%
...
WT=logs/CCVID/ccvid-34-1244/ez_eva02_vid_hybrid_extra_last.pth
...
CUDA_VISIBLE_DEVICES=0 python -W ignore -m torch.distributed.launch --nproc_per_node=$NUM_GPU --master_port $PORT \
    train.py --eval --resume --no-head --config_file $CONFIG DATA.ROOT $ROOT TEST.WEIGHT $WT SOLVER.SEED $SEED \
    TRAIN.TRAIN_VIDEO True DATA.DATASET $DATASET \
    MODEL.NAME 'ez_eva02_vid_hybrid_extra' TRAIN.COLOR_PROFILE $COLOR TEST.MODE True 
